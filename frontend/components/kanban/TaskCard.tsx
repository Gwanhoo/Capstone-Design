...
import { CalendarDays, Ellipsis, MessageSquare, Paperclip } from "lucide-react";
...
export function TaskCard({ task, isDragging = false, onClick, onDelete }: { task: Task; isDragging?: boolean; onClick: () => void; onDelete: () => void }) {
  ...
  return (
    <article onClick={onClick} className={`relative rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur-lg transition ${isDragging ? "scale-[1.02] border-primary/40 shadow-2xl" : "hover:border-primary/30 hover:shadow-lg"}`}>
      ...
      <div className="mt-3 flex items-center gap-3 text-[11px] text-outline"><span className="flex items-center gap-1"><MessageSquare className="h-3.5 w-3.5" />{task.comments}</span><span className="flex items-center gap-1"><Paperclip className="h-3.5 w-3.5" />{task.attachments}</span></div>
