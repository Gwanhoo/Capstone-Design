import { Bot, FileText, GanttChartSquare, LayoutDashboard, ListTodo, Settings, Sparkles } from "lucide-react";

const menus = [
  { label: "보드", icon: LayoutDashboard, active: true },
  { label: "백로그", icon: ListTodo },
  { label: "타임라인", icon: GanttChartSquare },
  ...
  return (
    <aside className="hidden h-screen w-64 flex-col border-r border-white/10 bg-surface-container-low px-4 py-6 md:flex">
      <div className="mb-6 px-2">
        ...
        <h2 className="mt-2 text-lg font-bold tracking-tight text-on-surface">AI 협업 칸반 프로젝트</h2>
      </div>
      ...
      <div className="mt-auto rounded-2xl border border-white/10 bg-surface-container-lowest/90 p-4 shadow-lg">
        ...
        <button className="flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-white/15 bg-white/5 text-xs font-semibold text-primary transition hover:bg-white/10">
