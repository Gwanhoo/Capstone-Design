...
export function KanbanHeader({ onGenerateAiTask }: { onGenerateAiTask: () => void }) {
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/10 bg-surface/70 px-4 backdrop-blur-xl md:px-6">
      <div><h1 className="text-base font-bold text-on-surface md:text-lg">AI 협업 프로젝트 · 메인 보드</h1><p className="text-xs text-on-surface-variant">실시간 태스크 흐름 · Sprint 12</p></div>
      <div className="flex items-center gap-3">
        ...
        <button onClick={onGenerateAiTask} className="hidden h-10 items-center gap-2 rounded-xl bg-[linear-gradient(135deg,#c3c0ff_0%,#4f46e5_100%)] px-4 text-sm font-semibold text-[#1d00a5] shadow-lg transition hover:brightness-110 md:inline-flex"><Sparkles className="h-4 w-4" />AI 작업 생성</button>
