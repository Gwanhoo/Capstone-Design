...
export function useKanbanBoard() {
  ...
  const moveTask = (targetColumnId: string, targetIndex?: number) => {
    ...
    setColumns((prev) => {
      ...
      const insertIndex = targetIndex ?? target.taskIds.length;
      target.taskIds.splice(insertIndex, 0, dragMeta.taskId);
      return next;
    });
    setDragMeta(null);
  ...
  const addTask = (columnId: string, task: Omit<Task, "id" | "progress" | "comments" | "attachments" | "aiStatus" | "assigneeInitial">) => {
    const id = `task-${Date.now()}`;
    const newTask: Task = { ...task, id, progress: 0, comments: 0, attachments: 0, aiStatus: "AI 리뷰 대기", assigneeInitial: task.assignee.slice(0, 1) };
    setTasks((prev) => ({ ...prev, [id]: newTask }));
    setColumns((prev) => prev.map((col) => (col.id === columnId ? { ...col, taskIds: [...col.taskIds, id] } : col)));
  };

  const updateTask = (taskId: string, partial: Partial<Task>) => setTasks((prev) => ({ ...prev, [taskId]: { ...prev[taskId], ...partial } }));
  ...
  const addAiGeneratedTask = () => {
    ...
    addTask("todo", { title, description: "AI 분석으로 생성된 추천 작업입니다.", priority: "높음", assignee: "AI 추천", dueDate: "2026.05.20" });
  };

  return { orderedColumns, tasks, dragMeta, dropColumnId, setDropColumnId, startDrag, moveTask, addTask, updateTask, deleteTask, addAiGeneratedTask };
}
